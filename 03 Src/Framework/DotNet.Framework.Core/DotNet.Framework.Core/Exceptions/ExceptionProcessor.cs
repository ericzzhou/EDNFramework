...
namespace DotNet.Framework.Core.Exceptions
{
    ...
    {
        public static string ExceptionType_System = "Application";
        public static string ExceptionType_Business = "Business";
        ...
        private static void OnNormalError(Entity.Exceptions.ServerLastError lastError)
        {
            ...
            if (lastError != null)
            {
                ...
                {
                    string businessExceptionDirectory = context.Server.MapPath("~/Logs/BusinessException");
                    WriteLog(businessExceptionDirectory, ex);
                    //业务异常直接返回
                    context.Response.Write(ex.Message);
                }
                else
                {
                    string applicationExceptionDirectory = context.Server.MapPath("~/Logs/ApplicatoinException");
                    WriteLog(applicationExceptionDirectory, ex);
                    //应用程序异常，直接返回，也可以读取类似.NET 默认错误黄页的页面模板返回效果。

                    string ErrorHtml = ReadTemplate(context.Server.MapPath("~/bin/Resources/DotNet.Framework.Core/Templates/ErrorPage.html"));

...
", orderField, dir);
      ...
      WHERE UserID=@UserID AND RoleID=@RoleID";
        ...
        public static PagingResult<IList<Entity_UserTableData>> GetListByPager(ViewQueryCondition<Entity_Role_Users_Condition> query)
        {
            PagingResult<IList<Entity_UserTableData>> result = new PagingResult<IList<Entity_UserTableData>>();
...
";
            Command cmdCount = CommandManager.CreateCommand(totleString);
            cmdCount.SetParamter("@RoleID", query.Condition.RoleID.Value);
...
", query.PagingInfo.OrderField, query.PagingInfo.SortDir);
            Command cmd = CommandManager.CreateCommand(sqlString);
            cmd.SetParamter("@RoleID", query.Condition.RoleID.Value);
            cmd.SetParamter("@PageIndex", query.PagingInfo.PageIndex);
            cmd.SetParamter("@PageSize", query.PagingInfo.PageSize);
            result.Result = cmd.ExecuteEntities<Entity_UserTableData>();

            return result;
        }

        public static PagingResult<IList<Entity_UserTableData>> GetListByPager(ViewQueryCondition<object> query)
        {
            PagingResult<IList<Entity_UserTableData>> result = new PagingResult<IList<Entity_UserTableData>>();
            result.PagingInfo = query.PagingInfo;
            string totleString = @"
...
", query.PagingInfo.OrderField, query.PagingInfo.SortDir);
        ...
        public static IList<int> GetUserRoles(int uid)
        {
            string sqlString = "SELECT roleid as RoleID FROM  EDNF_Account_UserRoles with(nolock) WHERE UserID = @UserID";
            Command cmd = CommandManager.CreateCommand(sqlString);
            cmd.SetParamter("@UserID", uid);
            return cmd.ExecuteEntities<Entity_Role>().Select(x=>x.RoleID).ToList();
        }
    }
}

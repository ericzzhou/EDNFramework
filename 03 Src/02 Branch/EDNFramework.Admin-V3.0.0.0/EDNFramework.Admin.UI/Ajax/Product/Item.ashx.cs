...
namespace DotNet.Framework.Admin.UI.Ajax.Product
{
    ...
    public class Item : IHttpHandler
    {
        ...
        private void Edit(HttpContext context)
        {
            ...
            if (Business.EDNFramework_Product.Business_Product_Class.HasSon(entity.ClassID))
            {
                throw new BusinessException(string.Format("产品类型【{0}】存在子类型，请直接在子类别添加。", Business.EDNFramework_Product.Business_Product_Class.GetModel(entity.ClassID).ClassName));
            }

...
";
            ...
            if (userId != 1)
            {
                sqlStr += "AND UR.UserID=@UserID ";
            }
            sqlStr += " ORDER BY EAP.Sequence DESC";


//"WHERE UR.UserID=@UserID ORDER BY EAP.Sequence DESC";
        ...
        public static PagingResult<IList<Entity_RoleList>> GetListByPager(ViewQueryCondition<object> query)
        {
            PagingResult<IList<Entity_RoleList>> result = new PagingResult<IList<Entity_RoleList>>();
...
", query.PagingInfo.OrderField, query.PagingInfo.SortDir);
            ...
            result.Result = cmd.ExecuteEntities<Entity_RoleList>();
            return result;
        }

        public static IList<Entity_RolePermissionAction_Table> GetRole_permissionActions(int roleID)
        {
            string sql = @"
SELECT  ( CASE WHEN EXISTS ( SELECT TOP 1
                                    ARPID
                             FROM   EDNF_Account_RolePermissions EARP
                             WHERE  EARP.PermissionCode = EAP.PermissionCode
                                    AND EARP.RoleID = @RoleID ) THEN 1
               ELSE 0
          END ) AS Checked ,
        EAP.PermissionName ,
        EAP.PermissionCode,
        ...
        RP.ARPID
FROM    EDNF_Account_Permission EAP WITH ( NOLOCK )
        LEFT JOIN EDNF_Account_PermissionCategories EAPC WITH ( NOLOCK ) ON EAPC.CategoryID = EAP.CategoryID
        LEFT JOIN EDNF_Account_RolePermissions RP WITH(NOLOCK) ON RP.RoleID=@RoleID AND RP.PermissionCode=EAP.PermissionCode
ORDER BY EAP.Sequence ASC 
";
            Command cmd = CommandManager.CreateCommand(sql);
            cmd.SetParamter("@RoleID", roleID);
            return cmd.ExecuteEntities<Entity_RolePermissionAction_Table>();
        }
    }
}

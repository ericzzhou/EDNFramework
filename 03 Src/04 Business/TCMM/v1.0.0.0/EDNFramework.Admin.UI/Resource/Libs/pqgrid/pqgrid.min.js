...
function PQLoadGrid(NormalSortColumn, obj_ID, url, colM, PageSize, topVisible, Condition) {
    ...
    var dataModel = {
        ...
        curPage: 1,
        rPP: PageSize,
        sortIndx: NormalSortColumn,
        sortDir: "up",
        rPPOptions: [10, 20, 30, 40, 50, 100, 500, 1000],
        getUrl: function () {
            var orderField = this.sortIndx;
            ...
            if (Condition) {
                FullCondition.Condition = Condition;
            }
            return {
                url: url,
                //data: "pageindex=" + this.curPage + "&pagesize=" + this.rPP
                //    + "&orderField=" + orderField + "&sortDir=" + sortDir
                data: JSON.stringify(FullCondition)
            };
        },
        getData: function (dataJSON) {
            if (dataJSON.totalRecords <= 0) {
                //$.AlertAutoClose("没有找到您要的相关数据！");
            }
            return {
    ...
    $(obj_ID).pqGrid({
        ...
        columnBorders: true,
        //freezeCols: 2,/*冻结列,需和 scrollModel 配合使用*/
        scrollModel: { pace: 'fast', horizontal: false },/*隐藏横向滚动条*/
        resizable: false,
        roundCorners: false,/*四个边角圆角显示*/
        rowBorders: true,
        flexWidth: false,/*宽度自动适应内容*/
        flexHeight: false,/*高度自动适应内容*/
        //electionModel: { type: 'cell', mode: 'block'},
        sortable: true,/*启用或禁用排序功能*/
        //width:800,
        wrap: true,/*单元格内文本过长是否自动换行*/
        numberCell: true,/*是否显示Number Cell 列*/
        numberCellWidth: 35,/*Number Cell 宽度*/
        minWidth: 50,/*最小列像素的宽度*/
        rowSelect: function (evt, ui) {
            GetRowIndex = ui.rowIndxPage;
            //GetRowIndexs += "," + ui.rowIndxPage;
            //GetRowIndexs[GetRowIndex];
        }
    });
    pqGridResize(obj_ID, -110, -30);
}
function PQLoadGridNoPage(NormalSortColumn, obj_ID, url, colM) {
    if (!ednframework || !ednframework.lib || !ednframework.lib.tools) {
        alert("请在之前引入ednframework.lib.js文件.");
        return;
    }
...
function pqGridRefresh(obj_id) {
    $(obj_id).pqGrid("refreshDataAndView");
    GetRowIndex = -1;
}
...
function GetPqGridCellValue(obj_ID, cellCode) {
    if (GetRowIndex != -1) {
        var DM = $(obj_ID).pqGrid("option", "dataModel");
        var data = DM.data;
        
        return data[GetRowIndex][cellCode];
    }
    else {
        return null;
    }
}
function GetPgGridRowData(obj_ID) {
    if (GetRowIndex != -1) {
        var DM = $(obj_ID).pqGrid("option", "dataModel");
        var data = DM.data;
